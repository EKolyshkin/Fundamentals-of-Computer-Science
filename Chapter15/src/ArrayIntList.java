...
public class ArrayIntList { // implements Iterable<Integer> {
 ...
 // EXERCISE CODE GOES HERE, to keep it organized, might be on a Quiz some day???
    ...
    {  // changed to non-static, used as member method now
        for (int i = elementData.length - 1; i >= 0; i--)
        {
            if (elementData[i] == value)
            { // changed array to elementData
                return i;
            }
        }
        return -1;
    ...
    // Exercise #4
    public void reverse()
    {   // switches the first half with the second half
        for (int i = 0; i < size / 2; i++)
        {
            int j = elementData[i]; // temp value
            elementData[i] = elementData[size - 1 - i];
            elementData[size - 1 - i] = j;
        } 
    }
